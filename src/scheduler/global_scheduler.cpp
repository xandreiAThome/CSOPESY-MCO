...
GlobalScheduler &GlobalScheduler::get() {
...
void GlobalScheduler::toggleProcessGeneration(bool enable) {
...
std::shared_ptr<Process> GlobalScheduler::getNextProcess(int coreId) {
...
bool GlobalScheduler::getProcessCache(const std::string &name,
                                      ProcessCache &outCache) {
  std::lock_guard<std::mutex> lock(mutex);
...
void GlobalScheduler::markWaiting(std::shared_ptr<Process> process) {
  std::lock_guard<std::mutex> lock(mutex);
...
void GlobalScheduler::markRunning(std::shared_ptr<Process> process) {
  std::lock_guard<std::mutex> lock(mutex);
...
void GlobalScheduler::markFinished(std::shared_ptr<Process> process) {
  std::lock_guard<std::mutex> lock(mutex);
...
std::string GlobalScheduler::buildProcessReport() {
  std::lock_guard<std::mutex> lock(mutex);
...
void GlobalScheduler::printProcessReport() {
  std::cout << buildProcessReport();
}

void GlobalScheduler::writeProcessReportToFile() {
  ...
  std::cout << "Report generated at " << std::filesystem::absolute(filename)
            << "!\n";
}

bool GlobalScheduler::processExists(const std::string &pName) {
  std::lock_guard<std::mutex> lock(mutex);
  return processExistsNoLock(pName);
}

void GlobalScheduler::createProcess(const std::string &name) {
...
void GlobalScheduler::wakeReadyProcesses() {
...
bool GlobalScheduler::allWorkersFinished() const {
  ...
  while (Globals::get().running) {
    ...
    if (generateProcesses.load()) {
      ...
      if (currentTick - lastGenerationTick >= freq) {
        ...
        std::string name = std::string("process_") + (newPid < 10 ? "0" : "") +
...
void GlobalScheduler::writeProcessInfo(
    std::ostream &out, const std::shared_ptr<Process> &process) {
  ...
  out << " | " << process->getExecutedInstructions() << " / "
      << process->getTotalInstructions() << "\n";
}

void GlobalScheduler::writeCpuStatus(std::ostream &out) {
  int coresUsed = 0;
...
void GlobalScheduler::writeFinishedProcessInfo(std::ostream &out,
                                               const ProcessCache &process) {
  out << "Name: " << process.name << " | Core: Finished"
      << " | " << process.executedInstructions << " / "
      << process.totalInstructions << "\n";
}

ProcessCache GlobalScheduler::createCacheFromProcess(
...
bool GlobalScheduler::processExistsNoLock(const std::string &pName) {
